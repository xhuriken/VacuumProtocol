...
/// <summary>
/// Description: Centralized manager for handling all player-internal collision ignoring rules (SSOT) pour le joueur V2.
...
/// </summary>
public class PlayerV2_CollisionManager : MonoBehaviour
{
    /// <summary>
    ...
    private void ClassifyColliders()
    {
        ...
        if (_enableDebugLogs)
        {
            Debug.Log($"[PlayerV2_CollisionManager] Classification Complete: " +
    ...
    private void ApplyCollisionRules()
    {
        ...
        IgnoreSelfCollisions(_neckColliders, ref ignoredPairs); // ADDED: Ignore self collisions inside neck!
        ...
        if (_enableDebugLogs)
        {
            Debug.Log($"[PlayerV2_CollisionManager] Applied {ignoredPairs} Physics.IgnoreCollision rules based on Inspector configuration.");
        }
    }
