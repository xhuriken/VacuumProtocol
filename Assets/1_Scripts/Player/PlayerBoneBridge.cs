...
public class PlayerBoneBridge : MonoBehaviour
{
    ...
    /// Justification: Must be done early so when gameplay scripts (like PlayerV2_Arms) initialize in Start(), the visual meshes are already aligned.
    /// </summary>
    private void Awake()
    {
        InitializeBridgeMap();
    ...
    private void PerformRuntimeBinding()
    {
        ...
        PlayerV2_Suspension suspension = GetComponentInParent<PlayerV2_Suspension>();
        if (suspension != null && suspension.WheelJoints != null)
        {
            foreach (ConfigurableJoint joint in suspension.WheelJoints)
            {
                if (joint != null) wheelTransforms.Add(joint.transform);
            }
        }
