...
public class UniVoiceMirrorSetupSample : MonoBehaviour
{
    ...
    /// <summary>
    /// Global reference to the local Voice Activity Detector.
    /// </summary>
    ...
    bool SetupClientSession()
    {
        ...
        {
            Mic.Device targetMic = null;
            if (SettingsManager.Instance != null && !string.IsNullOrEmpty(SettingsManager.Instance.CurrentSettings.ActiveMicrophoneDevice))
            {
                string savedDeviceName = SettingsManager.Instance.CurrentSettings.ActiveMicrophoneDevice;
                foreach (var dev in Mic.AvailableDevices)
                {
                    if (dev.Name == savedDeviceName)
                    {
                        targetMic = dev;
                        break;
                    }
            ...
            if (targetMic == null)
            {
                targetMic = Mic.AvailableDevices[0];
            }

            targetMic.StartRecording(60);
            Debug.Log($"[{TAG}] Started recording with Mic device: {targetMic.Name}");
            input = new UniMicInput(targetMic);
        }
        ...
        if (useVad)
        {
            LocalVad = new SimpleVad();
            ClientSession.InputFilters.Add(new SimpleVadFilter(LocalVad));
        }
